...
class Img_Dataset(Dataset):
    def __init__(self, n, height = 256, width = 256, subsize = 0, data_path = IMAGE_PATH):
        self.n = n
    ...
    def __getitem__(self, index):
        im_survey, im_obs = get_croped( id=self.listid[index], 
                                        ...
                                        data_path=self.data_path, metadata=self.metadata)
...
def get_croped(id, height = 256, width = 256, subsize = 0, data_path = IMAGE_PATH, metadata = METADATA):
    ...
    im_obs = read_image(str(obs_file))
    ...
    im_survey = read_image(str(survey_file))
    im_survey = crop(im_survey, top, left, crop_height, crop_width)
    if subsize > 0:
        height = height // subsize
        width = width // subsize
